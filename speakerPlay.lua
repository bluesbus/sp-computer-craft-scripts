...
-- main play function
local function playMusicOnSpeakers(speakers, file)
...
elseif start == "OggS" then
    return report_invalid_format("Ogg")
elseif start == "fLaC" then
    return report_invalid_format("FLAC")
elseif start:sub(1, 3) == "ID3" then
    return report_invalid_format("MP3")
elseif start == "<!DO" --[[<!DOCTYPE]] then
    return report_invalid_format("HTML")
end
...
while true do
    ...
    for _, speaker in ipairs(speakers) do
        speakerPlay(speaker, buffer)
    end
end
handle.close()
end

local function speakerPlay(speaker, audio)
